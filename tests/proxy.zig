    ...
    fn handle(f: *Fake, stream: net.Stream) void {
        ...
        if (f.chunked) {
            ...
            if (req.respondStreaming(&body_buf, .{
                ...
                var bw = bw_val;
                if (f.stream_gap_ms > 0) {
                    const step = f.payload.len / 3;
                    var at: usize = 0;
                    while (at < f.payload.len) {
                        const end = @min(at + step, f.payload.len);
                        bw.writer.writeAll(f.payload[at..end]) catch {};
                        bw.writer.flush() catch {};
                        writer.interface.flush() catch {};
                        f.io.sleep(.fromMilliseconds(@intCast(f.stream_gap_ms)), .awake) catch {};
                        at = end;
                    }
                } else {
...
test "a stream can be replayed at the speed it arrived" {
    const gpa = testing.allocator;
    var t: Io.Threaded = .init(gpa, .{});
    defer t.deinit();
    const io = t.io();

    const sse = try gpa.alloc(u8, 96 * 1024);
    defer gpa.free(sse);
    @memset(sse, 'd');

    var fake = try Fake.start(io, 39610, sse, 200);
    fake.chunked = true;
    fake.stream_gap_ms = 80;
    const ft = try std.Thread.spawn(.{}, Fake.serve, .{&fake});
    defer ft.join();
    defer fake.stop();

    const dir = ".tapedeck-proxy-pace";
    defer Io.Dir.cwd().deleteTree(io, dir) catch {};
    const path = dir ++ "/cassettes/default.jsonl";
    const base = try std.fmt.allocPrint(gpa, "http://127.0.0.1:{d}", .{fake.port});
    defer gpa.free(base);
    ...
    {
        var p = try Proxy.bind(gpa, io, path, .record, &ups, 39620);
        defer p.deinit();
        const th = try std.Thread.spawn(.{}, Proxy.serve, .{&p});
        const got = try callProxy(gpa, io, p.port, "/anthropic/v1/messages", body);
        defer gpa.free(got.body);
        try testing.expectEqualSlices(u8, sse, got.body);
        try p.flush();
        p.shutdown();
        th.join();
    }

    var saved = try Cassette.load(gpa, io, path);
    defer saved.deinit();
    const marks = saved.values()[0].timeline;
    try testing.expect(marks.len >= 2);
    try testing.expectEqual(sse.len, tapedeck.timeline.totalLen(marks));

    var recorded_ms: u64 = 0;
    for (marks) |m| recorded_ms += m.delay_ms;
    try testing.expect(recorded_ms >= 100);

    {
        var p = try Proxy.bind(gpa, io, path, .strict, &ups, 39630);
        p.replay_realtime = true;
        defer p.deinit();
        const th = try std.Thread.spawn(.{}, Proxy.serve, .{&p});

        const start_at = Io.Timestamp.now(io, .awake);
        const got = try callProxy(gpa, io, p.port, "/anthropic/v1/messages", body);
        defer gpa.free(got.body);
        const elapsed = @divTrunc(
            Io.Timestamp.now(io, .awake).nanoseconds - start_at.nanoseconds,
            std.time.ns_per_ms,
        );

        try testing.expectEqualSlices(u8, sse, got.body);
        try testing.expect(elapsed >= @divTrunc(@as(i96, @intCast(recorded_ms)), 2));
...
test "pacing is off by default so tests stay fast" {
    const gpa = testing.allocator;
    var t: Io.Threaded = .init(gpa, .{});
    defer t.deinit();
    const io = t.io();

    const dir = ".tapedeck-proxy-nopace";
    defer Io.Dir.cwd().deleteTree(io, dir) catch {};

    var p = try Proxy.bind(gpa, io, dir ++ "/cassettes/default.jsonl", .strict, &.{}, 39640);
    defer p.deinit();
    try testing.expect(!p.replay_realtime);
}
