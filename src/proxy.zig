...
pub const Proxy = struct {
    ...
    fn handle(p: *Proxy, req: *http.Server.Request) !void {
        ...
        if (!p.shouldRefresh(entry_key)) {
            if (p.lookup(entry_key)) |hit| {
                ...
                return p.respond(req, hit.status, hit.headers, hit.body, hit.chunked, hit.timeline);
            }
            if (p.mode == .strict) {
                p.bump(.missed);
    ...
    fn record(p: *Proxy, entry_key: []const u8, got: upstream.Response, u: usage_mod.Usage) !void {
        ...
        const e: Exchange = .{
            ...
            .chunked = got.chunked,
            .timeline = try gpa.dupe(timeline_mod.Chunk, got.timeline),
    ...
    fn respond(
        ...
        chunked: bool,
        marks: []const timeline_mod.Chunk,
    ) !void {
        const gpa = p.gpa;
        ...
        if (chunked) {
            ...
            if (p.replay_realtime and timeline_mod.totalLen(marks) == bytes.len) {
                var at: usize = 0;
                for (marks) |mark| {
                    if (mark.delay_ms > 0) p.io.sleep(.fromMilliseconds(mark.delay_ms), .awake) catch {};
                    try w.writer.writeAll(bytes[at..][0..mark.len]);
                    try w.writer.flush();
                    at += mark.len;
                }
            } else {
...
fn cloneExchange(gpa: std.mem.Allocator, e: Exchange) !Exchange {
    ...
    return .{
        ...
        .timeline = try gpa.dupe(timeline_mod.Chunk, e.timeline),
    };
}
