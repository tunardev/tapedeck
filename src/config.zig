...
test "pricing matches the longest configured prefix" {
    ...
    defer Io.Dir.cwd().deleteTree(io, dir) catch {};
    try writeConfig(io, dir,
        \\{"pricing":{"gpt-5.5":{"input":15,"output":75},
        \\            "gpt-5.5-2026-04-23":{"input":30,"output":150}}}
    );
    var c = try Config.load(testing.allocator, io, dir);
    defer c.deinit();

    try testing.expectApproxEqAbs(@as(f64, 15.0), c.cost("gpt-5.5", 1_000_000, 0).?, 0.0001);
    try testing.expectApproxEqAbs(@as(f64, 30.0), c.cost("gpt-5.5-2026-04-23", 1_000_000, 0).?, 0.0001);
    try testing.expect(c.cost("claude-opus-5", 1000, 1000) == null);
}
