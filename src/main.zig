...
const usage =
    ...
    \\  --realtime          replay a stream at the speed it was recorded
...
fn run(init: std.process.Init) !void {
    ...
    switch (inv.command) {
        ...
        .show => try show(gpa, io, home, cfg, inv.cassette),
...
fn wrap(
    ...
    proxy.replay_realtime = inv.realtime;
    defer proxy.deinit();
...
fn show(gpa: std.mem.Allocator, io: Io, home: Paths, cfg: config_mod.Config, name: []const u8) !void {
...
test "realtime is off unless asked for" {
    const off = try parse(&.{ "--strict", "--", "x" }, null);
    try testing.expect(!off.realtime);
    const on = try parse(&.{ "--strict", "--realtime", "--", "x" }, null);
    try testing.expect(on.realtime);
    try testing.expectEqual(proxy_mod.Mode.strict, on.mode);
}
