...
test "a dated model snapshot is not the same request as its alias" {
    try expectDiffers(
        \\{"model":"gpt-5.5-2026-04-23","messages":[]}
    ,
        \\{"model":"gpt-5.5","messages":[]}
    );
}

test "a date inside a model name survives scrubbing" {
    const gpa = testing.allocator;
    const k = try key(gpa, .{},
        \\{"model":"gpt-5.5-2026-04-23","messages":[]}
    , .{});
    defer gpa.free(k);
    try testing.expect(std.mem.indexOf(u8, k, "2026-04-23") != null);
}
